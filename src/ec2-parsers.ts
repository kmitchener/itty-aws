...
function parseByShape(node: any, shapeId: ShapeId): any {
  ...
  if (shape.kind === "list") {
    ...
    return nodes.map((n: any) =>
      parseByShape(n, shape.memberTarget as ShapeId),
    );
  }
...
export function parseEC2Response(xml: string): any {
  ...
  const rootElementName = Object.keys(root).find(
    (key) => !key.startsWith("?") && key.endsWith("Response"),
  );
  if (!rootElementName) {
  ...
  if (REGISTRY[fullShapeId]) {
    ...
    console.warn(
      `Shape ${fullShapeId} not found in registry, returning raw XML`,
    );
